...
def get_solutions(ids):
    ...
    for idx, i in enumerate(ids):
        if len(solutions) < 51:
            ...
            if failed_solution != 1:
                """
                <i class="fa fa-check-circle fa-green result-icon tool-tip"></i>
                //*[contains(@id,"submission-result")]/div/div[3]/table/tbody/tr[2]/td[2]/i

                //*[contains(@id="submission-result")]/div/div[3]/table/tbody/tr[2]/td[2]/i

                results = browser.find_elements_by_xpath('//*[contains(@id,"result-icon")]/td[2]/span')
                //*[@id="submission-result-3986202"]/div/div[3]/table/tbody/tr[5]/td[2]/span/i[1]
                """
                ...
                <i class="fa fa-check-circle fa-green result-icon tool-tip"></i>
                <i class="fa fa-times-circle fa-red result-icon tool-tip"></i>
                """
                ...
                link_key_between = re.search('src="/submission/key/(.*)/', html_content)
                link_key = link_key_between.group(1)
                solution_link = 'https://www.hackerearth.com/submission/key/' + link_key
                """
...
def get_description(i):
    """
    <div class="starwars-lab"><p>
    "\nSAMPLE INPUT\n"
    "\nSAMPLE OUTPUT\n"
    <div class="dark"><pre>
    "\nExplanation\n"
    <div class="less-margin">[0]

    ADD SCRIPT TO REMOVE PROBLEMS THAT USE IMAGES
    image problems contain:     src="https://d320jcjashajb2.cloudfront.net/media/uploads/be14d9c.jpg"
    #although it might be just decorative in some cases
    """
...
def get_descriptions(problem):
    ...
    #html_content = str(html_content_1[0]) + str(html_content_2[0]) + str(html_content_3[0])

    #less-margin-2 input-output-container
    #print html_content

    #print str(html_content[0])
    ...
    raw = raw.replace("\le", u"≤").replace("\ge", u"≥").replace("\lt", "<").replace("\gt", ">")

    #raw = re.sub('Subtasks(.+?)For', 'SAMPLE INPUT', str(raw), re.S)
    ...
    '''
    print html_content
    sub_soup = BeautifulSoup(html_content_all)
    hgd
    '''
...
def get_solutions(contest, solution_ids):
    ...
    for i in solution_ids:
        #url = 'http://codeforces.com/contest/686/submission/18671530'
        #url = 'http://codeforces.com/contest/676/submission/18233403'
        url = 'http://codeforces.com/contest/' + str(contest[0]) + '/submission/' + str(i)
        ...
        #text = soup.select("body a")
        #text = soup.select("#pageContent > div:nth-child(4) > pre")
        #text = soup.select("pageContent > div:nth-child(4) > pre")
        text = soup.select("body > div > div > div > div > pre")
        #text = soup.select("body > div > div > div > div")
        #text = soup.select(".prettyprint program-source")
        #text = soup.select("pre")
        ...
        #body = BeautifulSoup(text[0], "html.parser").get_text()


        '''
        body = BeautifulSoup(str(text[0]), "html.parser").get_text()

        print body.encode('utf-8').decode('string-escape')
...
def download_descriptions_solutions(filename, index_n):
    ...
    #problem_list = all_names[ndx]
    #descriptions, left_out, failed_to_download_d = get_descriptions(all_names)
    #descriptions, left_out, failed_to_download_d = get_descriptions(all_names)
